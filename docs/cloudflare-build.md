...
- Deploy command: `npm run deploy` (alias of `npm run deploy:cloudflare`)
...
3. Confirm deploy command is `npm run deploy`; it builds the Vue assets before invoking the pinned Wrangler version.
